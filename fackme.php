...
echo file_get_contents("https://core.telegram.org/bot" . API_KEY . "/setwebhook?url=" . $_SERVER['SERVER_NAME'] . "" . $_SERVER['SCRIPT_NAME']);
           function bot($method,$datas=[]){
$url = "https://core.telegram.org/bot".API_KEY."/".$method;
$ch = curl_init();
...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != 6324921017) {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$url_info = file_get_contents("https://core.telegram.org/bot".$ARM["BOTAPI_KEY"]."/getMe");
...
$admnb = [ 
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == 6324921017 ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
